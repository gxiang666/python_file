...
import requests
from openpyxl import Workbook  # 操作Excel的模块
import pymysql.cursors         # 操作mysql的模块


def get_conn():
    """建立数据库连接"""
    conn = pymysql.connect(host='39.96.39.7',
                                user='root',
                                password='root',
                                db='python',
                                charset='utf8mb4',
...
def get_json(url, page, lang_name):
    ...
    for i in list_con:
        info = []
        info.append(i.get('companyShortName', '无'))  # 无为默认值
